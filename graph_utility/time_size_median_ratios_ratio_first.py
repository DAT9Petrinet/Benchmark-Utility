...
def plot(data_list, test_names, graph_dir, experiment_to_compare_against_name):
    ...
    base_results['post_size'] = base_results.apply(
        lambda row: (row['post place count'] + row['post transition count']),
        axis=1)
    base_results.rename(columns=lambda x: f'base-{x}', inplace=True)

    data_list.pop(base_results_index)
    test_names.pop(base_results_index)

    def size_ratio(row):
        base_size = row['base-post_size']
        data_size = row['post place count'] + row['post transition count']
        if base_size == 0 and data_size == 0:
            return np.nan
        elif base_size == 0:
            return np.infty
        elif data_size == 0:
    ...
    for test_index, data in enumerate(data_list):
        ...
        data = pd.concat([data, base_results], axis=1)

        ratio_medians = dict()

        ratio_medians['overall post size median'] = data.apply(
            lambda row: size_ratio(row),
            axis=1).median()

        ratio_medians['overall time median'] = data.apply(
            lambda row: row['base-time'] /
                        row['time'],
            axis=1).median()

        ratio_medians['new rules used post size median'] = data.apply(
            lambda row: size_ratio(row)
            if new_rule_used(row, new_rules)
            else np.nan,
            axis=1).median()

        ratio_medians['new rules used time median'] = data.apply(
            lambda row: row['base-time'] /
                        row['time'] if new_rule_used(row, new_rules) else np.nan,
            axis=1).median()

        ratio_medians['no new rules used post size median'] = data.apply(
            lambda row: size_ratio(row)
            if not new_rule_used(row, new_rules)
            else np.nan,
            axis=1).median()

        ratio_medians['no new rules used time median'] = data.apply(
            lambda row: row['base-time'] /
                        row['time'] if not new_rule_used(row, new_rules) else np.nan,
            axis=1).median()

        ratio_df = pd.DataFrame(ratio_medians,
                                index=[f'{experiment_to_compare_against_name}/{test_names[test_index]}'])

        combined = combined.append(ratio_df)
        ...
        for p in plot.patches:
            ...
            if width > 0:
                plot.annotate(format(width, '.2f'), xy=(left + width, bottom + height / 2), ha='center', va='center',
                              size=10)
        plt.savefig(graph_dir + f'median_ratios_{png_names[index]}_ratio_first.png', bbox_inches='tight')
        plt.clf()
